...
def warp_x(x: float, s: float, seam_x: Optional[float], *, key: str, y: Optional[float]) -> float:
    """
    Default seam behavior (keeps a 1-unit connector column from x=seam-1..seam fixed at width 1):
      x' = x*s              for x < seam
      x' = x*s + (1-s)      for x >= seam

    Special fix for st:
      The bottom-right thick stroke of the 's' spans x=(seam-4)..seam in y~(15..19).
      If only x==seam gets the seam offset, that stroke fattens at wdth < 100.
      So: in that Y band, apply the same seam offset to x in [seam-4, seam) too,
      so the whole stroke shifts consistently (alignment) without changing thickness.
    """
    if seam_x is None:
        return x * s
    ...
    # Apply the default seam step at x>=seam
    if x >= seam_x - EPS:
        base += seam_off
    ...
    if key in ("st", "sh") and y is not None:
        ...
        if (y >= Y_MID1 - EPS) and (y <= Y_BASE0 + EPS):
            stroke_left = seam_x - 4.0  # thick-bar width at wdth=100
...
# -----------------------------
def transform_poly(poly: Poly, *, s: float, t: float, seam_x: Optional[float], key: str) -> Poly:
    r = is_axis_aligned_rect(poly)
    if r:
        x0, y0, x1, y1 = r
        w = x1 - x0
        ...
        nx0 = warp_x(x0, s, seam_x, key=key, y=y_mid)
        if abs(w - 1.0) < EPS:
            nx1 = nx0 + 1.0
        else:
            nx1 = warp_x(x1, s, seam_x, key=key, y=y_mid)
...
def build_master_ttf(
    *,
    ...
    for k in keys:
        ...
        warped_polys = [transform_poly(p, s=s, t=t, seam_x=sg.seam_x, key=k) for p in sg.polys]
        glyf[gname] = build_tt_glyph_from_polys(warped_polys, base_y_warped=base_y_warped)

        adv_svg = sg.width_svg + LETTER_SPACE_SVG
        # y=None => no st-only band logic for advance widths
        adv_warped = warp_x(adv_svg, s, sg.seam_x, key=k, y=None)
